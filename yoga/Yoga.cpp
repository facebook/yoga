...
float YGRoundValueToPixelGrid(
  ...
  if (fractial < 0) {
    ...
    //
    // Regarding `floor` and `ceil`. Note that for a number x, `floor(x) <= x <= ceil(x)` even for
    // negative numbers. Here are a couple of examples:
    //   - x =  2.2: floor( 2.2) =  2, ceil( 2.2) =  3
    //   - x = -2.2: floor(-2.2) = -3, ceil(-2.2) = -2
    //
    // Regarding `fmodf`. For fractional negative numbers, `fmodf` returns a negative
    // number. For example, `fmodf(-2.2) = -0.2`. However, we want `fractial` to be the
    // number such that subtracting it from `value` will give us `floor(value)`. In the
    // case of negative numbers, adding 1 to `fmodf(value)` gives us this. Let's continue
