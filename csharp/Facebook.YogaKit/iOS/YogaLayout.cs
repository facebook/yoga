...
namespace Facebook.YogaKit
{
	public partial class YogaLayout : NSObject
	{
		...
		private static partial void MeasureNativeView(UIView view, float constrainedWidth, float constrainedHeight, out float sizeThatFitsWidth, out float sizeThatFitsHeight)
		{
		...
		private static partial void GetWidthHeightOfNativeView(UIView view, out float width, out float height)
		{
			width = (float)view.Bounds.Width;
			height = (float)view.Bounds.Height;
		}

        private static partial float GetNativePixelScale() => (float)UIScreen.MainScreen.Scale;


		private static partial void ApplyLayoutToNativeView(UIView view, YogaNode node)
		{
