...
YGNodeListRef YGNodeListNew(const uint32_t initialCapacity) {
  ...
  if (list == NULL) {
    YGLog(NULL, YGLogLevelFatal, "Could not allocate memory for list");
  }
  ...
  if (list->items == NULL) {
    YGLog(NULL, YGLogLevelFatal, "Could not allocate memory for items");
  }
...
void YGNodeListInsert(YGNodeListRef *listp, const YGNodeRef node, const uint32_t index) {
  ...
  if (list->count == list->capacity) {
    ...
    if (list->items == NULL) {
      YGLog(NULL, YGLogLevelFatal, "Could not extend allocation for items");
    }
  }
