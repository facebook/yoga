...
function __transpileToJavaCommon(code) {
  return code
    ...
    .replace(/style\[CSS_START/g, 'style.position[POSITION_START')
    .replace(/style\[CSS_END/g, 'style.position[POSITION_END')
    ...
    .replace(/isLeadingPosDefined\((.+?),\s*(.+?)\)/g, '!isUndefined\($1.style.position.getWithFallback\(leadingSpacing[$2], leading[$2]\)\)')
    .replace(/isTrailingPosDefined\((.+?),\s*(.+?)\)/g, '!isUndefined\($1.style.position.getWithFallback\(trailingSpacing[$2], trailing[$2]\)\)')
    ...
    .replace(/getLeadingPosition\((.+?),\s*(.+?)\)/g, '\(isUndefined\($1.style.position.getWithFallback\(leadingSpacing[$2], leading[$2]\)\) ?  0 : $1.style.position.getWithFallback\(leadingSpacing[$2], leading[$2]\)\)')
    .replace(/getTrailingPosition\((.+?),\s*(.+?)\)/g, '\(isUndefined\($1.style.position.getWithFallback\(trailingSpacing[$2], trailing[$2]\)\) ?  0 : $1.style.position.getWithFallback\(trailingSpacing[$2], trailing[$2]\)\)')
    .replace(/setTrailingPosition\((.+?),\s*(.+?),\s*(.+?)\)/g, '$2.layout.position[trailing[$3]] = $1.layout.measuredDimensions[dim[$3]] - $2.layout.measuredDimensions[dim[$3]] - $2.layout.position[pos[$3]]')
